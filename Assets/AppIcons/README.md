...
Generated with the built-in `$imagegen` workflow for Cue.

## Files

- `Cue.icns`: selected app icon bundle generated from `app-icon-minimal-clean-play.png`.
...
- `script/build_and_run.sh` copies `Cue.icns` into the staged app bundle and declares it in `Info.plist`.
